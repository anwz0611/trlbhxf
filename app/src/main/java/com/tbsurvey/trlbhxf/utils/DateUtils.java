...
public class DateUtils {
    ...
    public static String convert(String start1) {
        long start = Long.parseLong(start1);
        long end = System.currentTimeMillis();
        long timeDifference = end - start;
        long second = timeDifference / (1000 * 60 * 60 * 24);
        long second1 = timeDifference / (1000);
        if (second >= 90) {
            return "";
        } else {
            String result = null;
            int d = (int)second1/86400;
            Date date = new Date((second1%86400)*1000);
            SimpleDateFormat df = new SimpleDateFormat("HH.mm.ss");
            df.setTimeZone(TimeZone.getTimeZone("GMT+0:00"));
            result =  d + "." + df.format(date);
            String[] b=result.split("\\.");
            return b[0]+"天"+b[1]+"小时"+b[2]+"分"+b[3]+"秒";
        }
